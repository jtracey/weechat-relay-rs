...
///
/// Note that while these pointers are probably parsed into an actual machine pointer type on the relay side, the protocol treats them as strings with particular constraints.
/// This means that there are possible cases where a relay and a client may have a different interpretation of things like whether two pointers are equal (e.g., leading zeros), or a pointer is valid at all (e.g., Pointers longer than the relay machine's pointer width).
/// Because the representation is only specified at the protocol level, we strongly advise to only use pointers as opaque handles provided by the relay.
...
pub enum PointerError {
    ...
    NonHex,
    /// The pointer string is too long to fit in a pointer (as of version 3.7 of the WeeChat Relay protocol, 255 bytes).
    TooLong,
}
...
impl Pointer {
    ...
    ///
    /// The `NULL` pointer is always an invalid identifier, using it in commands will yield empty responses.
    pub fn null() -> Pointer {
        Pointer(vec![b'0'])
    }
